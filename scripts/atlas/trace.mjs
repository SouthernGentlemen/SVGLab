...
function medianCut(pixels, count) {
  ...
  return boxes.filter((box) => box.length > 0).map((box) => {
    ...
    return total.map((sum) => Math.round(sum / box.length));
  });
}
 ...
 */
function refine(palette, distinct, rounds = 8) {
  const entries = [...distinct].map(([key, count]) => [(key >> 16) & 0xff, (key >> 8) & 0xff, key & 0xff, count]);
  let current = palette;
  for (let round = 0; round < rounds; round++) {
    const totals = current.map(() => [0, 0, 0, 0]);
    ...
    const next = totals.map((slot, index) => (slot[3] === 0
      ? current[index]
      : [Math.round(slot[0] / slot[3]), Math.round(slot[1] / slot[3]), Math.round(slot[2] / slot[3])]));
...
function quantise(part, colours, minRegionArea) {
  ...
  const samples = [...distinct.keys()].map((key) => [(key >> 16) & 0xff, (key >> 8) & 0xff, key & 0xff]);
 ...
 */
function denoise(indices, width, height) {
 ...
 */
function despeckle(indices, width, height, minArea, palette) {
...
function traceLoops(mask, width, height) {
  ...
  for (const [startKey, startList] of outgoing) {
    while (startList.length) {
      ...
      let direction = [x - startX, y - startY];
      let guard = 0;
 ...
 * melting into blobs.
 */
function loopToPath(loop, { smoothing, cornerAngle, originX, originY, scale }) {
  const n = loop.length;
  const at = (i) => loop[(i % n + n) % n];
  const parts = [];
  const cosLimit = Math.cos((180 - cornerAngle) * Math.PI / 180);

  const pointFor = (i, towards) => {
    const [x, y] = at(i);
  ...
  const pair = (x, y) => `${round((x - originX) * scale)} ${round((y - originY) * scale)}`.replace(" -", "-");
  const emit = (command, [x, y], control) => {
 ...
 */
export function tracePart(part, origin, options = {}) {
  const {
    colours = 22,
    epsilon = 0.7,
    smoothing = 0.9,
  ...
  const toPath = (mask, area = Infinity) => {
    ...
    return traceLoops(mask, width, height)
      ...
      .map((loop) => loopToPath(loop, { smoothing: smoothing * detail, cornerAngle, originX: origin.x, originY: origin.y, scale }))
      .join("");
  };
  ...
  const paths = [{ d: toPath(silhouette), fill: hex(palette[rim]), seam: 0 }];
  ...
  const rank = (blob) => (blob.members.length <= DETAIL_AREA ? Number.MAX_SAFE_INTEGER : blob.depth);
