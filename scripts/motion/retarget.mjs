...
function required(positions, name) {
...
function projectedAngle(positions, segment, previous) {
...
export function retargetClip(bvh, definition) {
  ...
  if (definition.loop) {
    ...
    if (seam > definition.maxLoopSeamDegrees) {
      throw new Error(`${definition.key}: ${round(seam)} degree loop seam exceeds ${definition.maxLoopSeamDegrees}`);
    }
  ...
  const targetFrame = (sampleIndex) => Math.round(sampleIndex / (poses.length - 1) * duration);
