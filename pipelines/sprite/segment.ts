...
export function findIslands({ width, height, data }: PixelImage): Segments {
...
function band(island: Island): number {
  const centreY = island.y + island.h / 2;
  return BAND_EDGES.findIndex((edge) => centreY < edge);
}

const byX = (a: Island, b: Island): number => (a.x + a.w / 2) - (b.x + b.w / 2);
 ...
 */
export function assignSlots(islands: readonly Island[], { source }: { readonly source: string }): Map<string, Island> {
  const bands: Island[][] = [[], [], [], []];
  ...
  const slots = new Map<string, Island>();
