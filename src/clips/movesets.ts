import { AUTHORED_CLIPS } from "./generated/authored.ts";
import { clipOrigin } from "./index.ts";
import type { ClipName } from "./index.ts";
...
function authoredReviewOptions(weapon: WeaponId, listed: readonly ClipName[]): PreviewClipOption[] {
  ...
  return (Object.keys(AUTHORED_CLIPS) as ClipName[])
    ...
    .filter((name) => {
      const origin = clipOrigin(name);
      return origin === null || loadout.has(origin);
    })
    .map((name) => ({ group: "authored" as const, slot: clipOrigin(name) ?? "new", clip: name }));
}
